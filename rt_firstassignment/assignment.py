...
def find_token_silver():
	...
	Returns:
		dist_silver (float): distance of the closest silver token (-1 if no silver token is detected)
		rot_y_silver (float): angle between the robot and the closest silver token (-1 if no silver token is detected)
		silver_number ( int): the number of silver token ( -1 if no silver token is detected)
	"""
	dist_silver=100
	silver_token=False # a boolean variable that is false if the robot doesn't find the silver token, otherwise it's true
	for token in R.see():
...
def find_token_gold():
	...
	Returns:
		...
		rot_y_gold (float): angle between the robot and the gold token (-1 if no gold token is detected)
		gold_number ( int): the number of gold token ( -1 if no gold token is detected)
	"""
	gold_token=False # a boolean variable that is false if the robot doesn't find the gold token, otherwise it's true
	for token in R.see():
...
def align_to_silver( dist_silver, rot_y_silver):
	"""
	Function for aligning robot to silver token
	...
	"""
	silver_grab=False # a boolean variable that is false if the robot doesn't grab the silver token, otherwise it's true
	if dist_silver==-1:
		turn(5,1)
...
def align_to_gold( dist_gold, rot_y_gold):
	"""
	Function for aligning robot to gold token
	...
	"""
	gold_reach=False #  a boolean variable that is false if the robot doesn't release the gold token, otherwise it's true
	if dist_gold==-1:
		turn(5,1)
...
def check_paired_silver( parray_silver, silver_number):
	...
	Args: parray_silver (array of integer): it contains the code number of paired silver tokens
	...
	j=0
	while j<6:
		if parray_silver[j]==silver_number: # if the current silver token has been already paired, the robot turns for finding a gold unpaired
...
def check_paired_gold( parray_gold, gold_number):
	...
	Args: parray_gold (array of integer): it contains the code number of paired gold tokens
...
def paired_tokens( parray_silver, silver_number, parray_gold, gold_number, count_pair):
	"""
	Function for pairing the tokens with their code number, that are passed as function arguments. In addition it checks if the robots has paired every silver token
	
	Args: parray_silver (array of integer): it contains the code number of paired silver tokens ( 0 if the silvers aren't paired)
	silver_number (int): the code number of current silver token
	parray_gold (array of integer): it contains the code number of paired gold tokens ( 0 if the golds aren't paired)
	gold_number (int): the code number of current gold token
	count_pair (int): the number of paired tokens ( 0 at the beggining)
	...
	if count_pair==6: # if every silver token has been paired, the program ends
		...
		drive(-20,1)
		exit()
	else: # if the previous condition is not verified, the robot moves and turns for finding the next silver token
		drive(-20,1)
		turn(25,2)
...
def main():
	parray_silver=[0]*6 # this array contains the code number of paired silver tokens (0 at the beggining) 
	parray_gold=[0]*6 # this array contains the code number of paired gold tokens (0 at the beggining)
	count_pair=0 # index for the access of previous arrays, and count the paired tokens
	silver_avaible=True # this bolean variable is true if any silver token is avaible ( at the beggining any silver token is avaible), otherwise it is false
	silver_taken=False # this bolean variable is true if the silver token is grabbed ( at the beggining any silver token is not grabbed), otherwise it is false
	gold_reach=False # this bolean variable is true if the gold token is found and reached ( at the beggining any gold token is not reached), otherwise it is false
	while 1:
		if silver_avaible:
